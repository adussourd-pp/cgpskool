...
- **Persistance limitée (RGPD)** : `CGP.project.PERSIST` dans `cgp-skool-core.js` liste les seuls modules autorisés à persister leur état (`bp-simulator`, `equipe-builder`, `productivite`, `udp-prep`, `tracking` — outils internes sans données client). Pour tous les autres, l'état passe par **sessionStorage** (effacé à la fermeture de l'onglet — permet le flux simulateur → dossier-placement pendant la session) : rien ne persiste sur l'appareil, conservation = JSON exporté manuellement. Un nettoyage au chargement purge les `cgpskool_state_<id>` non whitelistés stockés en localStorage par les anciennes versions.
